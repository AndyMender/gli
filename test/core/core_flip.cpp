...
int test_texture(const texture& Texture, const genType& ClearColor, const genType& FirstColor) {
  int Error = 0;
...
int main() {
  ...
  Error += test_texture(gli::texture2d(gli::FORMAT_R8_UNORM_PACK8, TextureSize, Levels), static_cast<glm::uint8>(255),
                        static_cast<glm::uint8>(0));

  Error += test_texture(gli::texture2d(gli::FORMAT_RGB8_UNORM_PACK8, TextureSize, Levels), glm::u8vec3(255, 128, 0),
  ...
  Error += test_texture(gli::texture2d_array(gli::FORMAT_RGBA32_SFLOAT_PACK32, TextureSize, 4, Levels),
                        glm::f32vec4(1.0, 0.5, 0.0, 1.0), glm::f32vec4(0.0, 0.5, 1.0, 1.0));
  ...
  Error +=
      test_texture(gli::texture2d(gli::FORMAT_RGBA_DXT5_UNORM_BLOCK16, TextureSize, Levels),
