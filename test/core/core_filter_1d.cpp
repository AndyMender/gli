...
namespace filter1d {
  int test() {
...

    {
      ...
      const gli::vec4 TexelA = Sampler.texture_lod(gli::fsampler1D::normalized_type(+0.5f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelA, ColorFill, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelB = Sampler.texture_lod(gli::fsampler1D::normalized_type(-0.5f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelB, ColorFill, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelC = Sampler.texture_lod(gli::fsampler1D::normalized_type(+1.5f), 0.0f);
    ...
    }
    {
      ...
      const gli::vec4 TexelB = Sampler.texture_lod(gli::fsampler1D::normalized_type(-1.0f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelB, ColorBorder, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelC = Sampler.texture_lod(gli::fsampler1D::normalized_type(+2.0f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelC, ColorBorder, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelD = Sampler.texture_lod(gli::fsampler1D::normalized_type(-0.5f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelD, (ColorFill + ColorBorder) * 0.5f, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelE = Sampler.texture_lod(gli::fsampler1D::normalized_type(1.5f), 0.0f);
    ...
    {
      gli::fsampler1D Sampler(Texture, gli::WRAP_CLAMP_TO_BORDER, gli::FILTER_NEAREST, gli::FILTER_NEAREST,
                              ColorBorder);

      const gli::vec4 TexelA = Sampler.texture_lod(gli::fsampler1D::normalized_type(0.5f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelA, ColorFill, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelB = Sampler.texture_lod(gli::fsampler1D::normalized_type(-0.4f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelB, ColorFill, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelC = Sampler.texture_lod(gli::fsampler1D::normalized_type(-0.5f), 0.0f);
      Error += gli::all(gli::epsilonEqual(TexelC, ColorBorder, 0.01f)) ? 0 : 1;

      const gli::vec4 TexelD = Sampler.texture_lod(gli::fsampler1D::normalized_type(-0.6f), 0.0f);
