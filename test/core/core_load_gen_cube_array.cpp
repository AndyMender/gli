...
int main() {
  ...
  for (int FormatIndex = gli::FORMAT_FIRST, FormatCount = gli::FORMAT_LAST; FormatIndex < FormatCount; ++FormatIndex) {
    const gli::target Target = gli::TARGET_CUBE_ARRAY;
    ...
    gli::texture TextureDDS(gli::load("test_cube_array.dds"));
    Error += (Texture == TextureDDS) ? 0 : 1;

    gli::save(Texture, "test_cube_array.ktx");
    gli::texture TextureKTX(gli::load("test_cube_array.ktx"));
    Error += (Texture == TextureKTX) ? 0 : 1;

    gli::save(Texture, "test_cube_array.kmg");
    gli::texture TextureKMG(gli::load("test_cube_array.kmg"));
