...
bool convert_rgb32f_rgb9e5(const char *FilenameSrc, const char *FilenameDst) {
  if (FilenameDst == nullptr) return false;

  // TOOO: use the C++ std::string::find instead?
  if (std::strstr(FilenameDst, ".dds") != nullptr || std::strstr(FilenameDst, ".ktx") != nullptr) return false;

  gli::texture2d TextureSource(gli::load(FilenameSrc));

  if (TextureSource.empty()) return false;

  if (TextureSource.format() != gli::FORMAT_RGB16_SFLOAT_PACK16 &&
      TextureSource.format() != gli::FORMAT_RGB32_SFLOAT_PACK32)
    return false;
...
namespace rgba_dxt1unorm {
  int test() {
    ...
    gli::texture2d TextureLocalDecompressed(TextureDecompressed.format(), TextureDecompressed.extent(),
                                            TextureDecompressed.levels(), TextureDecompressed.swizzles());
    ...
    for (size_t Level = 0; Level < TextureCompressed.levels(); ++Level) {
      ...
      for (BlockCoord.y = 0, TexelCoord.y = 0; BlockCoord.y < LevelExtentInBlocks.y;
           ++BlockCoord.y, TexelCoord.y += BlockExtent.y) {
        for (BlockCoord.x = 0, TexelCoord.x = 0; BlockCoord.x < LevelExtentInBlocks.x;
             ++BlockCoord.x, TexelCoord.x += BlockExtent.x) {
          const gli::detail::dxt1_block *DXT1Block = TextureCompressed.data<gli::detail::dxt1_block>(0, 0, Level) +
                                                     (BlockCoord.y * LevelExtentInBlocks.x + BlockCoord.x);
          ...
          for (DecompressedBlockCoord.y = 0; DecompressedBlockCoord.y < glm::min(4, LevelExtent.y);
               ++DecompressedBlockCoord.y) {
            for (DecompressedBlockCoord.x = 0; DecompressedBlockCoord.x < glm::min(4, LevelExtent.x);
                 ++DecompressedBlockCoord.x) {
              TextureLocalDecompressed.store(
                  TexelCoord + DecompressedBlockCoord, Level,
                  glm::u8vec4(glm::round(DecompressedBlock.Texel[DecompressedBlockCoord.y][DecompressedBlockCoord.x] *
                                         255.0f)));
            }
          }
        }
    ...
    for (int Level = 0; Level < TextureDecompressed.levels(); ++Level) {
      ...
      for (TexelCoord.y = 0; TexelCoord.y < LevelExtent.y; TexelCoord.y += glm::max(1, LevelExtent.y - 1)) {
        for (TexelCoord.x = 0; TexelCoord.x < LevelExtent.x; TexelCoord.x += glm::max(1, LevelExtent.x - 1)) {
          glm::u8vec4 ColorFromCompressed = glm::u8vec4(glm::round(
              gli::detail::convertFunc<gli::texture2d, float, 4, std::uint8_t, glm::defaultp,
                                       gli::detail::CONVERT_MODE_DXT1UNORM, true>::fetch(TextureCompressed, TexelCoord,
                                                                                         0, 0, Level) *
              255.0f));
  ...
  int test() {
    ...
    for (size_t Level = 0; Level < TextureCompressed.levels(); ++Level) {
      ...
      for (BlockCoord.y = 0, TexelCoord.y = 0; BlockCoord.y < LevelExtentInBlocks.y;
        ...
        for (BlockCoord.x = 0, TexelCoord.x = 0; BlockCoord.x < LevelExtentInBlocks.x;
             ++BlockCoord.x, TexelCoord.x += BlockExtent.x) {
          const gli::detail::dxt3_block *DXT3Block = TextureCompressed.data<gli::detail::dxt3_block>(0, 0, Level) +
                                                     (BlockCoord.y * LevelExtentInBlocks.x + BlockCoord.x);
    ...
    for (int Level = 0; Level < TextureDecompressed.levels(); ++Level) {
      ...
      for (TexelCoord.y = 0; TexelCoord.y < LevelExtent.y; TexelCoord.y += glm::max(1, LevelExtent.y - 1)) {
        for (TexelCoord.x = 0; TexelCoord.x < LevelExtent.x; TexelCoord.x += glm::max(1, LevelExtent.x - 1)) {
          glm::u8vec4 ColorFromCompressed = glm::u8vec4(glm::round(
              gli::detail::convertFunc<gli::texture2d, float, 4, std::uint8_t, glm::defaultp,
                                       gli::detail::CONVERT_MODE_DXT3UNORM, true>::fetch(TextureCompressed, TexelCoord,
                                                                                         0, 0, Level) *
              255.0f));
          glm::u8vec4 ColorFromDecompressed = TextureDecompressed.load<glm::u8vec4>(TexelCoord, Level);
  ...
  int test() {
    ...
    for (size_t Level = 0; Level < TextureCompressed.levels(); ++Level) {
      ...
      for (BlockCoord.y = 0, TexelCoord.y = 0; BlockCoord.y < LevelExtentInBlocks.y;
        ...
        for (BlockCoord.x = 0, TexelCoord.x = 0; BlockCoord.x < LevelExtentInBlocks.x;
             ++BlockCoord.x, TexelCoord.x += BlockExtent.x) {
          const gli::detail::dxt5_block *DXT5Block = TextureCompressed.data<gli::detail::dxt5_block>(0, 0, Level) +
                                                     (BlockCoord.y * LevelExtentInBlocks.x + BlockCoord.x);
    ...
    for (int Level = 0; Level < TextureDecompressed.levels(); ++Level) {
      ...
      for (TexelCoord.y = 0; TexelCoord.y < LevelExtent.y; TexelCoord.y += glm::max(1, LevelExtent.y - 1)) {
        for (TexelCoord.x = 0; TexelCoord.x < LevelExtent.x; TexelCoord.x += glm::max(1, LevelExtent.x - 1)) {
          glm::u8vec4 ColorFromCompressed = glm::u8vec4(glm::round(
              gli::detail::convertFunc<gli::texture2d, float, 4, std::uint8_t, glm::defaultp,
                                       gli::detail::CONVERT_MODE_DXT5UNORM, true>::fetch(TextureCompressed, TexelCoord,
                                                                                         0, 0, Level) *
              255.0f));
          glm::u8vec4 ColorFromDecompressed = TextureDecompressed.load<glm::u8vec4>(TexelCoord, Level);
  ...
  int test() {
    ...
    for (size_t Level = 0; Level < TextureCompressed.levels(); ++Level) {
      ...
      for (BlockCoord.y = 0, TexelCoord.y = 0; BlockCoord.y < LevelExtentInBlocks.y;
        ...
        for (BlockCoord.x = 0, TexelCoord.x = 0; BlockCoord.x < LevelExtentInBlocks.x;
             ++BlockCoord.x, TexelCoord.x += BlockExtent.x) {
          const gli::detail::bc4_block *BC4Block = TextureCompressed.data<gli::detail::bc4_block>(0, 0, Level) +
                                                   (BlockCoord.y * LevelExtentInBlocks.x + BlockCoord.x);
          ...
          for (DecompressedBlockCoord.y = 0; DecompressedBlockCoord.y < glm::min(4, LevelExtent.y);
            ...
            for (DecompressedBlockCoord.x = 0; DecompressedBlockCoord.x < glm::min(4, LevelExtent.x);
                 ++DecompressedBlockCoord.x) {
              glm::u8vec4 rgbaTexel(
                  glm::round(DecompressedBlock.Texel[DecompressedBlockCoord.y][DecompressedBlockCoord.x] * 255.0f));
    ...
    for (int Level = 0; Level < TextureDecompressed.levels(); ++Level) {
      ...
      for (TexelCoord.y = 0; TexelCoord.y < LevelExtent.y; TexelCoord.y += glm::max(1, LevelExtent.y - 1)) {
        for (TexelCoord.x = 0; TexelCoord.x < LevelExtent.x; TexelCoord.x += glm::max(1, LevelExtent.x - 1)) {
          glm::u8vec4 ColorFromCompressed = glm::u8vec4(glm::round(
              gli::detail::convertFunc<gli::texture2d, float, 4, std::uint8_t, glm::defaultp,
                                       gli::detail::CONVERT_MODE_BC4UNORM, true>::fetch(TextureCompressed, TexelCoord,
                                                                                        0, 0, Level) *
              255.0f));
  ...
  int test() {
    ...
    for (size_t Level = 0; Level < TextureCompressed.levels(); ++Level) {
      ...
      for (BlockCoord.y = 0, TexelCoord.y = 0; BlockCoord.y < LevelExtentInBlocks.y;
        ...
        for (BlockCoord.x = 0, TexelCoord.x = 0; BlockCoord.x < LevelExtentInBlocks.x;
             ++BlockCoord.x, TexelCoord.x += BlockExtent.x) {
          const gli::detail::bc5_block *BC5Block = TextureCompressed.data<gli::detail::bc5_block>(0, 0, Level) +
                                                   (BlockCoord.y * LevelExtentInBlocks.x + BlockCoord.x);
          ...
          for (DecompressedBlockCoord.y = 0; DecompressedBlockCoord.y < glm::min(4, LevelExtent.y);
            ...
            for (DecompressedBlockCoord.x = 0; DecompressedBlockCoord.x < glm::min(4, LevelExtent.x);
              ...
              glm::u8vec2 decompressed =
                  TextureDecompressed.load<glm::u8vec2>(TexelCoord + DecompressedBlockCoord, Level);

              if (glm::xy(rgbaTexel) == decompressed) {
                ++onPoint;
              } else if (glm::abs((int32_t)rgbaTexel.r - (int32_t)decompressed.r) > 1 ||
                         glm::abs((int32_t)rgbaTexel.g - (int32_t)decompressed.g) > 1) {
                ++offByAlot;
              } else {
                ++offByOne;
              }
    ...
    for (int Level = 0; Level < TextureDecompressed.levels(); ++Level) {
      ...
      for (TexelCoord.y = 0; TexelCoord.y < LevelExtent.y; TexelCoord.y += glm::max(1, LevelExtent.y - 1)) {
        for (TexelCoord.x = 0; TexelCoord.x < LevelExtent.x; TexelCoord.x += glm::max(1, LevelExtent.x - 1)) {
          glm::u8vec4 ColorFromCompressed = glm::u8vec4(glm::round(
              gli::detail::convertFunc<gli::texture2d, float, 4, std::uint8_t, glm::defaultp,
                                       gli::detail::CONVERT_MODE_BC5UNORM, true>::fetch(TextureCompressed, TexelCoord,
                                                                                        0, 0, Level) *
              255.0f));
