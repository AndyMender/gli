...
namespace {
	// TODO: where does SOURCE_DIR come from? use a const or constexpr instead?
	// TODO: use a C++ string instead of a C-string as input?
...
namespace load_mem_only {
  int test(const std::vector<char>& Data, const std::string& Filename) {
    int Error = 0;
...
int main() {
  ...
  {
    for (const auto& filename: Filenames) Error += load_file::test(filename);
  }
  std::clock_t TimeFileEnd = std::clock();

  std::clock_t TimeMemStart = std::clock();
  {
    for (const auto& filename: Filenames) Error += load_mem::test(filename);
  }
  ...
  std::cout << "File: " << TimeFileEnd - TimeFileStart << ", "
			<< "Mem: " << TimeMemEnd - TimeMemStart << ", "
			<< "Mem Only: " << TimeMemOnlyEnd - TimeMemOnlyStart
			<< std::endl;
