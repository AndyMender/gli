...
namespace gli {
  ...
  class image {
   ...
   public:
    ...
    explicit image(format_type Format, const extent_type& Extent);
    ...
    explicit image(const image& Image, format_type Format);
    ...
    /// Clear the entire image storage_linear with Texel which type must match the image storage_linear format block
    /// size If the type of genType doesn't match the type of the image format, no conversion is performed and the data
    /// will be reinterpreted as if is was of the image format.
    template <typename genType>
    void clear(const genType& Texel);
    ...
    template <typename genType>
    genType load(const extent_type& TexelCoord);

    /// Store the texel located at TexelCoord coordinates.
    /// It's an error to call this function if the format is compressed.
    /// It's an error if TexelCoord values aren't between [0, dimensions].
    template <typename genType>
    void store(const extent_type& TexelCoord, const genType& Data);
   ...
   private:
    ...
    /// with a different compatible image format.
    explicit image(std::shared_ptr<storage_linear> Storage, format_type Format, size_type BaseLayer, size_type BaseFace,
                   size_type BaseLevel);

    std::shared_ptr<storage_linear> Storage;
    const format_type Format;
    const size_type BaseLevel;
    data_type* Data;
