...
namespace gli {
  ...
  class sampler1d_array : public sampler {
   ...
   public:
    using texture_type = texture1d_array;
    ...
    sampler1d_array(const texture_type& Texture, wrap Wrap, filter Mip = FILTER_NEAREST, filter Min = FILTER_NEAREST,
    ...
    void texel_write(const extent_type& TexelCoord, size_type layer, size_type Level, texel_type const& Texel);
  ...
  using fsampler1DArray = sampler1d_array<float>;
  using dsampler1DArray = sampler1d_array<double>;
  using usampler1DArray = sampler1d_array<unsigned int>;
  using isampler1DArray = sampler1d_array<int>;
}  // namespace gli
