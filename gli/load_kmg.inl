...
namespace gli {
  namespace detail {
    const static unsigned char FOURCC_KMG100[] = {0xAB, 0x4B, 0x49, 0x4D, 0x20, 0x31,
    ...
    inline texture load_kmg100(const char* Data, std::size_t Size) {
      const detail::kmgHeader10& Header(*reinterpret_cast<const detail::kmgHeader10*>(Data));

      size_t Offset = sizeof(detail::kmgHeader10);

      texture Texture(
          static_cast<target>(Header.Target), static_cast<format>(Header.Format),
          texture::extent_type(Header.PixelWidth, Header.PixelHeight, Header.PixelDepth), Header.Layers, Header.Faces,
          Header.Levels,
      ...
      for (texture::size_type Layer = 0, Layers = Texture.layers(); Layer < Layers; ++Layer)
        for (texture::size_type Level = 0, Levels = Texture.levels(); Level < Levels; ++Level) {
          const texture::size_type FaceSize = static_cast<texture::size_type>(Texture.size(Level));
      ...
      return texture(Texture, Texture.target(), Texture.format(), Texture.base_layer(), Texture.max_layer(),
                     Texture.base_face(), Texture.max_face(), Header.BaseLevel, Header.MaxLevel, Texture.swizzles());
    }
  }  // namespace detail

  inline texture load_kmg(const char* Data, std::size_t Size) {
  ...
  inline texture load_kmg(const char* Filename) {
  ...
  inline texture load_kmg(const std::string& Filename) { return load_kmg(Filename.c_str()); }
}  // namespace gli
