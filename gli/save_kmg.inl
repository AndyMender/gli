...
namespace gli {
  inline bool save_kmg(const texture& Texture, std::vector<char>& Memory) {
  ...
  inline bool save_kmg(const texture& Texture, char const* Filename) {
    if (Texture.empty()) return false;
  ...
  inline bool save_kmg(const texture& Texture, const std::string& Filename) {
