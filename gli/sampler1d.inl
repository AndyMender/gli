...
namespace gli {
  ...
  inline sampler1d<T, P>::sampler1d(const texture_type& Texture, wrap Wrap, filter Mip, filter Min,
  ...
  template <typename T, qualifier P>
  inline typename sampler1d<T, P>::texel_type sampler1d<T, P>::texel_fetch(const extent_type& TexelCoord,
  ...
  template <typename T, qualifier P>
  inline void sampler1d<T, P>::texel_write(const extent_type& TexelCoord, const size_type& Level,
  ...
  template <typename T, qualifier P>
  inline void sampler1d<T, P>::clear(const texel_type& Color) {
    GLI_ASSERT(!this->Texture.empty());
    GLI_ASSERT(this->Convert.Write);
  ...
  template <typename T, qualifier P>
  inline typename sampler1d<T, P>::texel_type sampler1d<T, P>::texture_lod(const normalized_type& SampleCoord,
                                                                           level_type Level) const {
    GLI_ASSERT(!this->Texture.empty());
    GLI_ASSERT(std::numeric_limits<T>::is_iec559);
    GLI_ASSERT(this->Filter && this->Convert.Fetch);

    const normalized_type SampleCoordWrap(this->Wrap(SampleCoord.x));
