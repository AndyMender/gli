...
namespace gli {
  ...
  inline vec<4, T, P> make_vec4(const vec<1, T, P>& v) {
    return vec<4, T, P>(v.x, static_cast<T>(0), static_cast<T>(0), static_cast<T>(1));
  }

  template <typename T, qualifier P>
  inline vec<4, T, P> make_vec4(const vec<2, T, P>& v) {
    return vec<4, T, P>(v.x, v.y, static_cast<T>(0), static_cast<T>(1));
  }

  template <typename T, qualifier P>
  inline vec<4, T, P> make_vec4(const vec<3, T, P>& v) {
    return vec<4, T, P>(v.x, v.y, v.z, static_cast<T>(1));
  }

  template <typename T, qualifier P>
  inline vec<4, T, P> make_vec4(const vec<4, T, P>& v) {
    return v;
  }
}  // namespace gli
