...
namespace gli {
  namespace detail {
    inline texture::size_type compute_ktx_storage_size(texture const& Texture) {
      const texture::size_type BlockSize = block_size(Texture.format());
      ...
      for (texture::size_type Level = 0, Levels = Texture.levels(); Level < Levels; ++Level) {
        ...
        for (texture::size_type Layer = 0, Layers = Texture.layers(); Layer < Layers; ++Layer)
          for (texture::size_type Face = 0, Faces = Texture.faces(); Face < Faces; ++Face) {
            ...
            const texture::size_type PaddedSize =
                std::max(BlockSize, glm::ceilMultiple(FaceSize, static_cast<texture::size_type>(4)));
  ...
  inline bool save_ktx(const texture& Texture, std::vector<char>& Memory) {
    if (Texture.empty()) return false;

    gl GL(gl::PROFILE_KTX);
    const gl::format& Format = GL.translate(Texture.format(), Texture.swizzles());
    const target Target = Texture.target();
    ...
    for (texture::size_type Level = 0, Levels = Texture.levels(); Level < Levels; ++Level) {
      ...
      for (texture::size_type Layer = 0, Layers = Texture.layers(); Layer < Layers; ++Layer)
        for (texture::size_type Face = 0, Faces = Texture.faces(); Face < Faces; ++Face) {
          ...
          const texture::size_type PaddedSize = glm::ceilMultiple(FaceSize, static_cast<texture::size_type>(4));
  ...
  inline bool save_ktx(const texture& Texture, char const* Filename) {
    if (Texture.empty()) return false;
    ...
    const bool Result = save_ktx(Texture, Memory);
  ...
  inline bool save_ktx(const texture& Texture, const std::string& Filename) {
