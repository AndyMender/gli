/// @brief Include to use generic textures which can represent any texture target but they don't have target specific
/// built-in caches making accesses slower.
...
namespace gli {
  ...
  class texture {
   public:
    ...
    using target_type = gli::target;
    using format_type = gli::format;
    using swizzles_type = gli::swizzles;
    using storage_type = storage_linear;
    using data_type = storage_type::data_type;
    using extent_type = storage_type::extent_type;
    ...
    /// @param Swizzles A mechanism to swizzle the components of a texture before they are applied according to the
    /// texture environment.
    texture(target_type Target, format_type Format, const extent_type& Extent, size_type Layers, size_type Faces,
            size_type Levels,
    ...
    texture(const texture& Texture, target_type Target, format_type Format, size_type BaseLayer, size_type MaxLayer,
    ...
    /// This texture object is effectively a texture view where the target and format can be reinterpreted
    /// with a different compatible texture target and texture format.
    texture(const texture& Texture, target_type Target, format_type Format,
            const swizzles_type& Swizzles = swizzles_type(SWIZZLE_RED, SWIZZLE_GREEN, SWIZZLE_BLUE, SWIZZLE_ALPHA));
    ...
    /// Return the base layer of the texture instance, effectively a memory offset in the actual texture storage_type to
    /// identify where to start reading the layers.
    size_type base_layer() const;

    /// Return the max layer of the texture instance, effectively a memory offset to the beginning of the last layer in
    /// the actual texture storage_type that the texture instance can access.
    size_type max_layer() const;

    /// Return max_layer() - base_layer() + 1
    size_type layers() const;

    /// Return the base face of the texture instance, effectively a memory offset in the actual texture storage_type to
    /// identify where to start reading the faces.
    size_type base_face() const;

    /// Return the max face of the texture instance, effectively a memory offset to the beginning of the last face in
    /// the actual texture storage_type that the texture instance can access.
    ...
    /// Return the base level of the texture instance, effectively a memory offset in the actual texture storage_type to
    /// identify where to start reading the levels.
    size_type base_level() const;

    /// Return the max level of the texture instance, effectively a memory offset to the beginning of the last level in
    /// the actual texture storage_type that the texture instance can access.
    ...
    /// Clear the entire texture storage_linear with Texel which type must match the texture storage_linear format block
    /// size If the type of gen_type doesn't match the type of the texture format, no conversion is performed and the
    /// data will be reinterpreted as if is was of the texture format.
    template <typename gen_type>
    void clear(const gen_type& Texel);

    /// Clear a specific image of a texture.
    template <typename gen_type>
    void clear(size_type Layer, size_type Face, size_type Level, const gen_type& BlockData);

    /// Clear a subset of a specific image of a texture.
    template <typename gen_type>
    void clear(size_type Layer, size_type Face, size_type Level, const extent_type& TexelOffset,
               const extent_type& TexelExtent, const gen_type& BlockData);

    /// Copy a specific image of a texture
    void copy(const texture& TextureSrc, size_t LayerSrc, size_t FaceSrc, size_t LevelSrc, size_t LayerDst,
              size_t FaceDst, size_t LevelDst);

    /// Copy a subset of a specific image of a texture
    void copy(const texture& TextureSrc, size_t LayerSrc, size_t FaceSrc, size_t LevelSrc, const extent_type& OffsetSrc,
              size_t LayerDst, size_t FaceDst, size_t LevelDst, const extent_type& OffsetDst,
              const extent_type& Extent);

    /// Reorder the component in texture memory.
    template <typename gen_type>
    void swizzle(const gli::swizzles& Swizzles);
   ...
   protected:
    ...
    struct cache {
     public:
      ...
      cache(storage_type& Storage, format_type Format, size_type BaseLayer, size_type Layers, size_type BaseFace,
            size_type MaxFace, size_type BaseLevel, size_type MaxLevel)
          : Faces(MaxFace - BaseFace + 1), Levels(MaxLevel - BaseLevel + 1) {
          ...
          for (size_type Face = 0; Face < this->Faces; ++Face)
            for (size_type Level = 0; Level < this->Levels; ++Level) {
              ...
              this->BaseAddresses[Index] =
                  Storage.data() + Storage.base_offset(BaseLayer + Layer, BaseFace + Face, BaseLevel + Level);
            }

        for (size_type Level = 0; Level < this->Levels; ++Level) {
          const extent_type& SrcExtent = Storage.extent(BaseLevel + Level);
          const extent_type& DstExtent = SrcExtent * block_extent(Format) / Storage.block_extent();
      ...
      // In texels
      extent_type get_extent(size_type Level) const { return this->ImageExtent[Level]; };

      // In bytes
      size_type get_memory_size(size_type Level) const { return this->ImageMemorySize[Level]; };
