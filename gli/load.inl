...
namespace gli {
  ...
  inline texture load(const char* Data, std::size_t Size) {
    {
  ...
  inline texture load(const char* Filename) {
  ...
  /// Load a texture (DDS, KTX or KMG) from file
  inline texture load(const std::string& Filename) { return load(Filename.c_str()); }
}  // namespace gli
