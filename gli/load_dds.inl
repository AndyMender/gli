...
namespace gli {
  namespace detail {
    const static char FOURCC_DDS[] = {'D', 'D', 'S', ' '};
    ...
    enum dds_surface_flag { DDSCAPS_COMPLEX = 0x00000008, DDSCAPS_MIPMAP = 0x00400000, DDSCAPS_TEXTURE = 0x00001000 };
    ...
    inline target get_target(const dds_header& Header, const dds_header10& Header10) {
      if ((Header.CubemapFlags & detail::DDSCAPS2_CUBEMAP) ||
          (Header10.MiscFlag & detail::D3D10_RESOURCE_MISC_TEXTURECUBE)) {
  ...
  inline texture load_dds(const char* Data, std::size_t Size) {
    GLI_ASSERT(Data && (Size >= sizeof(detail::FOURCC_DDS)));

    if (strncmp(Data, detail::FOURCC_DDS, 4) != 0) return texture();
    std::size_t Offset = sizeof(detail::FOURCC_DDS);

    GLI_ASSERT(Size >= sizeof(detail::dds_header));

    const detail::dds_header& Header(*reinterpret_cast<const detail::dds_header*>(Data + Offset));
    Offset += sizeof(detail::dds_header);

    detail::dds_header10 Header10;
    if ((Header.Format.flags & dx::DDPF_FOURCC) &&
        (Header.Format.fourCC == dx::D3DFMT_DX10 || Header.Format.fourCC == dx::D3DFMT_GLI1)) {
    ...
    if ((Header.Format.flags &
         (dx::DDPF_RGB | dx::DDPF_ALPHAPIXELS | dx::DDPF_ALPHA | dx::DDPF_YUV | dx::DDPF_LUMINANCE)) &&
        Format == gli::FORMAT_UNDEFINED && Header.Format.bpp != 0) {
      ...
      const dx::d3dfmt FourCC = detail::remap_four_cc(Header.Format.fourCC);
    ...
    const size_t MipMapCount = (Header.Flags & detail::DDSD_MIPMAPCOUNT) ? Header.MipMapLevels : 1;
    size_t FaceCount = 1;
    if (Header.CubemapFlags & detail::DDSCAPS2_CUBEMAP)
    ...
    size_t DepthCount = 1;
    if (Header.CubemapFlags & detail::DDSCAPS2_VOLUME) DepthCount = Header.Depth;

    texture Texture(get_target(Header, Header10), Format, texture::extent_type(Header.Width, Header.Height, DepthCount),
                    std::max<texture::size_type>(Header10.ArraySize, 1), FaceCount, MipMapCount);

    const std::size_t SourceSize = Offset + Texture.size();
  ...
  inline texture load_dds(const char* Filename) {
  ...
  inline texture load_dds(const std::string& Filename) { return load_dds(Filename.c_str()); }
}  // namespace gli
