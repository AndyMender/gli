...
namespace gli {
  namespace detail {
    ...
    inline void flip_block_s3tc(uint8_t* BlockDst, uint8_t* BlockSrc, format Format, bool HeightTwo) {
      ...
      // TODO: use enum classes to avoid global scope pollution
      // DXT1
      ...
      // DXT5 
      else if ((Format == FORMAT_RGBA_DXT5_UNORM_BLOCK16) || (Format == FORMAT_RGBA_DXT5_SRGB_BLOCK16)) {
    	...
    	// TODO: should this really break the entire stack?
    ...
    inline void flip_s3tc(image ImageDst, image ImageSrc, format Format) {
      ...
      const std::size_t XBlocks = ImageSrc.extent().x <= 4 ? 1 : ImageSrc.extent().x / 4;
      if (ImageSrc.extent().y == 2) {
        for (std::size_t i_block = 0; i_block < XBlocks; ++i_block)
          flip_block_s3tc(ImageDst.data<uint8_t>() + i_block * block_size(Format),
                          ImageSrc.data<uint8_t>() + i_block * block_size(Format), Format, true);

        return;
      }

      const std::size_t MaxYBlock = ImageSrc.extent().y / 4 - 1;
      for (std::size_t i_row = 0; i_row <= MaxYBlock; ++i_row)
        for (std::size_t i_block = 0; i_block < XBlocks; ++i_block) {
          flip_block_s3tc(
              ImageDst.data<uint8_t>() + (MaxYBlock - i_row) * block_size(Format) * XBlocks +
                  i_block * block_size(Format),
              ImageSrc.data<uint8_t>() + i_row * block_size(Format) * XBlocks + i_block * block_size(Format), Format,
              false);
        }
    }
  ...
  /*
  template <>
  inline image flip(const image& Image)
  {
  ...
  */
  ...
  template <>
  inline texture2d flip(const texture2d& Texture) {
    ...
    if (!is_compressed(Texture.format())) {
      const texture2d::size_type BlockSize = block_size(Texture.format());
  ...
  template <>
  inline texture2d_array flip(const texture2d_array& Texture) {
    GLI_ASSERT(!gli::is_compressed(Texture.format()) || gli::is_s3tc_compressed(Texture.format()));

    texture2d_array Flip(Texture.format(), Texture.extent(), Texture.layers(), Texture.levels());

    if (!gli::is_compressed(Texture.format())) {
      const texture2d_array::size_type BlockSize = block_size(Texture.format());
  ...
  template <>
  inline texture_cube flip(const texture_cube& Texture) {
    GLI_ASSERT(!gli::is_compressed(Texture.format()) || gli::is_s3tc_compressed(Texture.format()));

    texture_cube Flip(Texture.format(), Texture.extent(), Texture.levels());

    if (!gli::is_compressed(Texture.format())) {
      const texture_cube::size_type BlockSize = block_size(Texture.format());
  ...
  template <>
  inline texture_cube_array flip(const texture_cube_array& Texture) {
    ...
    if (!is_compressed(Texture.format())) {
      const gli::size_t BlockSize = block_size(Texture.format());
  ...
  template <>
  inline texture flip(const texture& Texture) {
    switch (Texture.target()) {
      ...
      default:
    	// TODO: boolean statement between bool and string!
    	// TODO: should this really break the stack?
