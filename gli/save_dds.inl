...
namespace gli {
  namespace detail {
    ...
    inline dx::d3dfmt get_fourcc(bool RequireDX10Header, gli::format Format, const dx::format& DXFormat) {
      if (RequireDX10Header) {
        const detail::formatInfo& FormatInfo = detail::get_format_info(Format);
  ...
  inline bool save_dds(const texture& Texture, std::vector<char>& Memory) {
    if (Texture.empty()) return false;

    dx DX;
    const dx::format& DXFormat = DX.translate(Texture.format());

    const bool RequireDX10Header = DXFormat.D3DFormat == dx::D3DFMT_GLI1 || DXFormat.D3DFormat == dx::D3DFMT_DX10 ||
                                   is_target_array(Texture.target()) || is_target_1d(Texture.target());

    Memory.resize(Texture.size() + sizeof(detail::FOURCC_DDS) + sizeof(detail::dds_header) +
    ...
    const detail::formatInfo& Desc = detail::get_format_info(Texture.format());
    ...
    Header.Pitch =
        static_cast<std::uint32_t>((Desc.Flags & detail::CAP_COMPRESSED_BIT) ? Texture.size() / Texture.faces() : 32);
    ...
    // Texture3D
    if (Texture.extent().z > 1) Header.CubemapFlags |= detail::DDSCAPS2_VOLUME;
  ...
  inline bool save_dds(const texture& Texture, char const* Filename) {
    if (Texture.empty()) return false;
  ...
  inline bool save_dds(const texture& Texture, const std::string& Filename) {
