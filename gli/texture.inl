...
namespace gli {
  ...
  inline texture::texture(target_type Target, format_type Format, const extent_type& Extent, size_type Layers,
                          size_type Faces, size_type Levels, const swizzles_type& Swizzles)
      : Storage(std::make_shared<storage_type>(Format, Extent, Layers, Faces, Levels)),
        ...
        MaxLevel(Levels - 1),
        Swizzles(Swizzles),
        Cache(*Storage, Format, this->base_layer(), this->layers(), this->base_face(), this->max_face(),
  ...
  inline texture::texture(const texture& Texture, target_type Target, format_type Format, size_type BaseLayer,
                          size_type MaxLayer, size_type BaseFace, size_type MaxFace, size_type BaseLevel,
                          size_type MaxLevel, const swizzles_type& Swizzles)
    ...
    GLI_ASSERT(Target != TARGET_1D || (Target == TARGET_1D && this->layers() == 1 && this->faces() == 1 &&
                                       this->extent().y == 1 && this->extent().z == 1));
    GLI_ASSERT(Target != TARGET_1D_ARRAY || (Target == TARGET_1D_ARRAY && this->layers() >= 1 && this->faces() == 1 &&
                                             this->extent().y == 1 && this->extent().z == 1));
    GLI_ASSERT(Target != TARGET_2D || (Target == TARGET_2D && this->layers() == 1 && this->faces() == 1 &&
                                       this->extent().y >= 1 && this->extent().z == 1));
    GLI_ASSERT(Target != TARGET_2D_ARRAY || (Target == TARGET_2D_ARRAY && this->layers() >= 1 && this->faces() == 1 &&
                                             this->extent().y >= 1 && this->extent().z == 1));
    GLI_ASSERT(Target != TARGET_3D || (Target == TARGET_3D && this->layers() == 1 && this->faces() == 1 &&
                                       this->extent().y >= 1 && this->extent().z >= 1));
    GLI_ASSERT(Target != TARGET_CUBE || (Target == TARGET_CUBE && this->layers() == 1 && this->faces() >= 1 &&
                                         this->extent().y >= 1 && this->extent().z == 1));
    GLI_ASSERT(Target != TARGET_CUBE_ARRAY || (Target == TARGET_CUBE_ARRAY && this->layers() >= 1 &&
                                               this->faces() >= 1 && this->extent().y >= 1 && this->extent().z == 1));
  }

  inline texture::texture(const texture& Texture, target_type Target, format_type Format, const swizzles_type& Swizzles)
      : Storage(Texture.Storage),
        Target(Target),
        Format(Format),
  ...
  inline bool texture::empty() const {
    if (this->Storage.get() == nullptr) return true;

    return this->Storage->empty();
  }

  inline texture::format_type texture::format() const { return this->Format; }

  inline texture::swizzles_type texture::swizzles() const {
    const swizzles_type FormatSwizzle = detail::get_format_info(this->format()).Swizzles;
    const swizzles_type CustomSwizzle = this->Swizzles;
  ...
  inline texture::size_type texture::base_layer() const { return this->BaseLayer; }

  inline texture::size_type texture::max_layer() const { return this->MaxLayer; }
  ...
  inline texture::size_type texture::base_face() const { return this->BaseFace; }

  inline texture::size_type texture::max_face() const { return this->MaxFace; }
  ...
  inline texture::size_type texture::base_level() const { return this->BaseLevel; }

  inline texture::size_type texture::max_level() const { return this->MaxLevel; }
  ...
  template <typename gen_type>
  inline void texture::clear(const gen_type& Texel) {
    GLI_ASSERT(!this->empty());
    GLI_ASSERT(block_size(this->format()) == sizeof(gen_type));

    gen_type* Data = this->data<gen_type>();
    const size_type BlockCount = this->size<gen_type>();

    for (size_type BlockIndex = 0; BlockIndex < BlockCount; ++BlockIndex) *(Data + BlockIndex) = Texel;
  }

  template <typename gen_type>
  inline void texture::clear(size_type Layer, size_type Face, size_type Level, const gen_type& BlockData) {
    GLI_ASSERT(!this->empty());
    GLI_ASSERT(block_size(this->format()) == sizeof(gen_type));
    GLI_ASSERT(Layer >= 0 && Layer < this->layers() && Face >= 0 && Face < this->faces() && Level >= 0 &&
               Level < this->levels());

    const size_type BlockCount = this->Storage->level_size(Level) / sizeof(gen_type);
    gen_type* Data = this->data<gen_type>(Layer, Face, Level);

    for (size_type BlockIndex = 0; BlockIndex < BlockCount; ++BlockIndex) *(Data + BlockIndex) = BlockData;
  }

  template <typename gen_type>
  inline void texture::clear(size_type Layer, size_type Face, size_type Level, const extent_type& TexelOffset,
                             const extent_type& TexelExtent, const gen_type& BlockData) {
    const storage_type::size_type BaseOffset = this->Storage->base_offset(Layer, Face, Level);
    ...
    const extent_type BlockExtent(TexelExtent / this->Storage->block_extent() + BlockOffset);
  ...
  inline void texture::copy(const texture& TextureSrc, size_t LayerSrc, size_t FaceSrc, size_t LevelSrc,
    ...
    memcpy(this->data(LayerDst, FaceDst, LevelDst), TextureSrc.data(LayerSrc, FaceSrc, LevelSrc), this->size(LevelDst));
  }

  inline void texture::copy(const texture& TextureSrc, size_t LayerSrc, size_t FaceSrc, size_t LevelSrc,
                            const texture::extent_type& OffsetSrc, size_t LayerDst, size_t FaceDst, size_t LevelDst,
                            const texture::extent_type& OffsetDst, const texture::extent_type& Extent) {
    const storage_type::extent_type BlockExtent = this->Storage->block_extent();
    this->Storage->copy(*TextureSrc.Storage, LayerSrc, FaceSrc, LevelSrc, OffsetSrc / BlockExtent, LayerDst, FaceDst,
                        LevelDst, OffsetDst / BlockExtent, Extent / BlockExtent);
  }

  template <typename gen_type>
  inline void texture::swizzle(const gli::swizzles& Swizzles) {
    for (size_type TexelIndex = 0, TexelCount = this->size<gen_type>(); TexelIndex < TexelCount; ++TexelIndex) {
      ...
      const gen_type TexelSrc = TexelDst;
  ...
  template <typename gen_type>
  inline gen_type texture::load(const extent_type& TexelCoord, size_type Layer, size_type Face, size_type Level) const {
    GLI_ASSERT(!this->empty());
    GLI_ASSERT(!is_compressed(this->format()));
    GLI_ASSERT(block_size(this->format()) == sizeof(gen_type));

    const size_type ImageOffset = this->Storage->image_offset(TexelCoord, this->extent(Level));
  ...
  template <typename gen_type>
  inline void texture::store(const extent_type& TexelCoord, size_type Layer, size_type Face, size_type Level,
