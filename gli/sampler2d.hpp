...
namespace gli {
  ...
  class sampler2d : public sampler {
   ...
   public:
    ...
    sampler2d(const texture_type& Texture, wrap Wrap, filter Mip = FILTER_NEAREST, filter Min = FILTER_NEAREST,
    ...
    texel_type texture_grad(const normalized_type& SampleCoord, const normalized_type& dPdx,
  ...
  using fsampler2D = sampler2d<float>;
  using dsampler2D = sampler2d<double>;
  using usampler2D = sampler2d<unsigned int>;
  using isampler2D = sampler2d<int>;
}  // namespace gli
