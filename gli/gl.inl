...
namespace gli {
  namespace detail {
    inline gl::swizzles translate(const gli::swizzles& Swizzles) {
    ...
    enum format_property { FORMAT_PROPERTY_BGRA_FORMAT_BIT = (1 << 0), FORMAT_PROPERTY_BGRA_TYPE_BIT = (1 << 1) };
  }  // namespace detail

  inline gl::gl(profile Profile) : Profile(Profile) {
    const bool HasSwizzle = has_swizzle(Profile);
    const external_format ExternalBGR = HasSwizzle ? EXTERNAL_RGB : EXTERNAL_BGR;
    const external_format ExternalBGRA = HasSwizzle ? EXTERNAL_RGBA : EXTERNAL_BGRA;
    const external_format ExternalBGRInt = HasSwizzle ? EXTERNAL_RGB_INTEGER : EXTERNAL_BGR_INTEGER;
    const external_format ExternalBGRAInt = HasSwizzle ? EXTERNAL_RGBA_INTEGER : EXTERNAL_BGRA_INTEGER;

    const external_format ExternalSRGB8 = Profile != PROFILE_ES20 ? EXTERNAL_RGB : EXTERNAL_SRGB_EXT;
    const external_format ExternalSRGB8_A8 = Profile != PROFILE_ES20 ? EXTERNAL_RGBA : EXTERNAL_SRGB_ALPHA_EXT;

    const internal_format InternalBGRA = Profile == PROFILE_ES20 ? INTERNAL_BGRA8_UNORM : INTERNAL_RGBA8_UNORM;
    const internal_format InternalRGBETC = Profile == PROFILE_ES20 ? INTERNAL_RGB_ETC : INTERNAL_RGB_ETC2;

    const internal_format InternalLuminance8 = HasSwizzle ? INTERNAL_R8_UNORM : INTERNAL_LUMINANCE8;
    const internal_format InternalAlpha8 = HasSwizzle ? INTERNAL_R8_UNORM : INTERNAL_ALPHA8;
    const internal_format InternalLuminanceAlpha8 = HasSwizzle ? INTERNAL_RG8_UNORM : INTERNAL_LUMINANCE8_ALPHA8;

    const internal_format InternalLuminance16 = HasSwizzle ? INTERNAL_R16_UNORM : INTERNAL_LUMINANCE16;
    const internal_format InternalAlpha16 = HasSwizzle ? INTERNAL_R16_UNORM : INTERNAL_ALPHA16;
    const internal_format InternalLuminanceAlpha16 = HasSwizzle ? INTERNAL_RG16_UNORM : INTERNAL_LUMINANCE16_ALPHA16;

    const external_format ExternalLuminance = HasSwizzle ? EXTERNAL_RED : EXTERNAL_LUMINANCE;
    const external_format ExternalAlpha = HasSwizzle ? EXTERNAL_RED : EXTERNAL_ALPHA;
    const external_format ExternalLuminanceAlpha = HasSwizzle ? EXTERNAL_RG : EXTERNAL_LUMINANCE_ALPHA;

    const type_format TypeF16 = Profile == PROFILE_ES20 ? TYPE_F16_OES : TYPE_F16;

    const format_desc Table[] = {
        ...
        {INTERNAL_RGB8_USCALED_GTC, ExternalBGR, TYPE_U8,
         detail::FORMAT_PROPERTY_BGRA_FORMAT_BIT},  // FORMAT_BGR8_USCALED_PACK8,
        {INTERNAL_RGB8_SSCALED_GTC, ExternalBGR, TYPE_I8,
         detail::FORMAT_PROPERTY_BGRA_FORMAT_BIT},  // FORMAT_BGR8_SSCALED_PACK8,
        ...
        {INTERNAL_RGBA8_SNORM, ExternalBGRA, TYPE_I8,
         detail::FORMAT_PROPERTY_BGRA_FORMAT_BIT},  // FORMAT_BGRA8_SNORM_PACK8,
        {INTERNAL_RGBA8_USCALED_GTC, ExternalBGRA, TYPE_U8,
         detail::FORMAT_PROPERTY_BGRA_FORMAT_BIT},  // FORMAT_BGRA8_USCALED_PACK8,
        {INTERNAL_RGBA8_SSCALED_GTC, ExternalBGRA, TYPE_I8,
         detail::FORMAT_PROPERTY_BGRA_FORMAT_BIT},  // FORMAT_BGRA8_SSCALED_PACK8,
        ...
        {INTERNAL_RGB10A2_UNORM, EXTERNAL_RGBA, TYPE_UINT32_RGB10A2,
         detail::FORMAT_PROPERTY_BGRA_TYPE_BIT},  // FORMAT_BGR10A2_UNORM_PACK32,
        {INTERNAL_RGB10A2_SNORM_EXT, EXTERNAL_RGBA, TYPE_UINT32_RGB10A2,
         detail::FORMAT_PROPERTY_BGRA_TYPE_BIT},  // FORMAT_BGR10A2_SNORM_PACK32,
        {INTERNAL_RGB10A2_USCALED_GTC, EXTERNAL_RGBA, TYPE_UINT32_RGB10A2,
         detail::FORMAT_PROPERTY_BGRA_TYPE_BIT},  // FORMAT_BGR10A2_USCALE_PACK32,
        {INTERNAL_RGB10A2_SSCALED_GTC, EXTERNAL_RGBA, TYPE_UINT32_RGB10A2,
         detail::FORMAT_PROPERTY_BGRA_TYPE_BIT},  // FORMAT_BGR10A2_SSCALE_PACK32,
        {INTERNAL_RGB10A2U, EXTERNAL_RGBA_INTEGER, TYPE_UINT32_RGB10A2,
         detail::FORMAT_PROPERTY_BGRA_TYPE_BIT},  // FORMAT_BGR10A2_UINT_PACK32,
        {INTERNAL_RGB10A2I_EXT, EXTERNAL_RGBA_INTEGER, TYPE_UINT32_RGB10A2,
         detail::FORMAT_PROPERTY_BGRA_TYPE_BIT},  // FORMAT_BGR10A2_SINT_PACK32,
  ...
  inline const gl::target& gl::translate(gli::target Target) const {
    static const gl::target Table[] = {gl::TARGET_1D,         gl::TARGET_1D_ARRAY, gl::TARGET_2D,
  ...
  inline gl::format gl::translate(gli::format Format, const gli::swizzles& Swizzles) const {
    GLI_ASSERT(Format >= FORMAT_FIRST && Format <= FORMAT_LAST);

    const gl::format_desc& FormatDesc = this->FormatDesc[Format - FORMAT_FIRST];
  ...
  inline gli::format gl::find(gl::internal_format InternalFormat, gl::external_format ExternalFormat,
                              gl::type_format Type) {
    for (int FormatIndex = FORMAT_FIRST; FormatIndex <= FORMAT_LAST; ++FormatIndex) {
      const std::size_t Index = FormatIndex - FORMAT_FIRST;
      if (this->FormatDesc[Index].Internal != InternalFormat) continue;
      if (this->FormatDesc[Index].External != ExternalFormat) continue;
      if (this->FormatDesc[Index].Type != Type) continue;
  ...
  inline gl::swizzles gl::compute_swizzle(const format_desc& FormatDesc, const gli::swizzles& Swizzles) const {
    if (!this->has_swizzle(this->Profile))
      return swizzles(gl::SWIZZLE_RED, gl::SWIZZLE_GREEN, gl::SWIZZLE_BLUE, gl::SWIZZLE_ALPHA);

    const bool IsExternalBGRA =
