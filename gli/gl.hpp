...
namespace gli {
  ...
  class gl {
   public:
    ...
    enum profile { PROFILE_ES20, PROFILE_ES30, PROFILE_GL32, PROFILE_GL33, PROFILE_KTX };
    ...
    const target& translate(gli::target Target) const;

    /// Convert GLI formats into OpenGL texture formats
    format translate(gli::format Format, const gli::swizzles& Swizzle) const;
   ...
   private:
    ...
    bool has_swizzle(profile Profile) const { return Profile == PROFILE_ES30 || Profile == PROFILE_GL33; }

    gl::swizzles compute_swizzle(const format_desc& FormatDesc, const gli::swizzles& Swizzle) const;
